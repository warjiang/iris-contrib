// Copy from github.com/kataras/iris/v12@v12.2.0/sessions
package sessions

import (
	"github.com/warjiang/iris-contrib/sessions/sessiondb/mysql"
	"sync"
	"testing"
	"time"
...
func TestSessions(t *testing.T) {
	...
	sess := sessions.New(sessions.Config{
		Cookie:  "mycustomsessionid",
		Expires: time.Hour * 24 * 30,
	})
	db := mysql.New(mysql.Options{
		Host:      "127.0.0.1",
		Port:      3306,
		Username:  "root",
		Password:  "123456",
...
func TestSessionsUpdateExpirationConcurrently(t *testing.T) {
	...
	i := 0
	wg := sync.WaitGroup{}
	// limit by mysql connection,we have test 150 concurrent at max
	n := 150
	wg.Add(n)
	for i < n {
		go func() {
